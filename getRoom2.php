<?php
	...
	session_start();
		...
		//if($p1==$p2 && $p2==$p3 && $p3==$p4 && $p4==$p5 && $p5==$g1 && $g1==$g2){
			//echo "panju";
			$query = "SELECT * FROM rent WHERE city LIKE '%$c%' AND (price>='{$a}' AND price<='{$b}');";
		}
		else{
			//echo "bhaiya";
			$query = "(SELECT * FROM rent WHERE (city LIKE '%$c%') AND (price>='{$a}' AND price<='{$b}') AND (type='{$p1}' OR type='{$p2}' OR type='{$p3}' OR type='{$p4}') AND (furniture='{$g1}' OR furniture='{$g2}' OR furniture='{$g3}')) ;";
		}
		...
		while($row = mysql_fetch_assoc($r)){ ?>
			<div class="col-sm-4 col-lg-4 col-md-4">
                        <div class="thumbnail">
                            <a href="indRoom.php?oid=<?php echo $row['oid']; ?>"><img src="http://placehold.it/320x150" alt="<?php echo $row['name']; ?>"></a>
                            <a href="indRoom.php?oid=<?php echo $row['oid']; ?>"></a>
                            <div class="caption">
                                <h4 class="pull-right"><?php echo $row['price']; ?>
</h4>
                                <h4><a href="indRoom.php?oid=<?php echo $row['oid']; ?>"><?php echo $row['name']; ?></a>
                                </h4>
                                <p>
